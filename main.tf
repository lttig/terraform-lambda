...
# Create S3 bucket for storing lambda function
resource "random_pet" "lambda_bucket_name" {
  prefix = "python-lambda-function"
  length = 4
}
...
resource "aws_s3_bucket_object" "simple_lambda" {
  bucket = aws_s3_bucket.lambda_bucket.id
  key    = "function.zip"
  source = data.archive_file.simple_lambda.output_path
  etag = filemd5(data.archive_file.simple_lambda.output_path)
}
...
resource "aws_lambda_function" "simple_lambda" {
  ...
  s3_key    = aws_s3_bucket_object.simple_lambda.key
...
resource "aws_apigatewayv2_integration" "simple_lambda" {
  api_id = aws_apigatewayv2_api.lambda.id
...
resource "aws_apigatewayv2_route" "simple_lambda" {
  api_id = aws_apigatewayv2_api.lambda.id

  route_key = "$default"
  target    = "integrations/${aws_apigatewayv2_integration.simple_lambda.id}"
}
